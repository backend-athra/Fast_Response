...
exports.createWork = async (req, res) => {
  try {
    ...
    const clientUser = await User.findById(clientId).select("fcmToken");
 if (clientUser?.fcmToken) {
    ...
    if (assignedTech) {
      ...
      await sendNotification(
        ...
        "technician",
        "New Work Assigned",
        `You have been assigned a new ${serviceType} job.`,
        "work_assigned",
        `work-${work.token}`
      );

      const techUser = await User.findById(assignedTech).select("fcmToken");

      if (techUser?.fcmToken) {

        await admin.messaging().send({
          token: techUser.fcmToken,

          notification: {
            title: "New Work Assigned",
            body: `You have a new ${serviceType} service request.`,
          },
        ...
        console.log("✅ TECHNICIAN WORK_ASSIGNED PUSH SENT");
      }
    }
...
exports.bookTechnician = async (req, res) => {
  try {
    ...
    const lockedWork = await Work.findOneAndUpdate(
      { _id: workId, assignedTechnician: { $in: [null, undefined] } },
      { assignedTechnician: technicianId, status: "approved" },
      { new: true }
    );

    if (!lockedWork) {
      return res.status(400).json({ message: "Work already assigned" });
    }

    const [client, technician] = await Promise.all([
      User.findById(userId).select("firstName fcmTokens address"),
      User.findById(technicianId).select("firstName fcmTokens")
    ]);
    ...
    lockedWork.otp = otp;
    lockedWork.otpCreatedAt = new Date();

    await lockedWork.save();
    ...
    if (client?.fcmTokens?.length) {
      ...
      await sendPush(
        client.fcmTokens,
        "Service OTP",
        `Your service OTP is ${otp}. Share this with technician.`,
        {
          role: "TECHNICIAN",
          type: "WORK_OTP",
          workId: lockedWork._id.toString(),
          otp: otp
        },
        client._id
      );

      console.log("✅ OTP PUSH SENT TO CLIENT");
...
console.log(otp)
    await sendNotification(
      ...
      "technician",
      "New Work Assigned",
      `${client.firstName} booked you for ${serviceType}`,
      "new_work",
    ...
    if (technician?.fcmTokens?.length) {
      ...
      console.log("✅ TECHNICIAN PUSH SENT");
  ...
  try {
    ...
    if (String(work.assignedTechnician) !== String(technicianId)) {
      return res.status(403).json({
        message: "Not assigned technician"
      });
    }
    ...
    // OTP verified
    work.otpVerified = true;
    
    work.workStartedAt = new Date();
...
exports.WorkStart = async (req, res) => {
  ...
  try {
    ...
    const work = await Work.findById(workId)
      .populate("client", "firstName email phone fcmToken")
    ...
    if (String(work.assignedTechnician?._id) !== String(technicianId)) {
      return res.status(403).json({
        message: "Unauthorized: not your assigned work"
      });
    }
    ...
    await sendNotification(
      ...
      "client",
      "Payment Successful",
      `Your ${work.serviceType} payment has been confirmed.`,
      "payment_received",
      `work-${work.token}`
    );
    ...
    if (work.client?.fcmToken) {
      ...
      await admin.messaging().send({
        ...
        data: {
          ...
          body: String("Technician confirmed your service payment"),
...
exports.stopTracking = async (req, res) => {
  try {
    ...
    const work = await Work.findOne({
      assignedTechnician: technicianId,
      status: { $in: ["dispatch","on_the_way","inprogress"] }
    }).populate("client", "coordinates");

    if (!work) {
      return res.status(404).json({
        message: "No active work found"
      });
    }
    ...
    work.trackingDistanceStatus = distanceStatus;

    await work.save();

    if (global.io) {
      global.io.emit(`tracking-stopped-${technicianId}`, {
        workId: work._id,
        technicianId,
        stoppedBy: "technician",
        distanceMeters,
        distanceStatus,
...
exports.reachLocation = async (req, res) => {
  try {
  ...
  console.log(req.body)
    ...
    if (clientUser?.fcmTokens?.length) {
      ...
      await sendPush(
        clientUser.fcmTokens,
        "Technician Arrived",
        "Your technician has arrived at your location. Please share the OTP to start the work.",
        {
          role: "CLIENT",
    ...
    if (global.io) {
      global.io.emit(`technician-arrived-${work._id}`, {
        workId: work._id,
        technicianId,
        status: "arrived",
        arrivedAt: Date.now()
    ...
    return res.status(200).json({
      success: true,
      message: "Technician reached location",
      workId: work._id
    });

  } catch (err) {

    console.error("Reach Location Error:", err);
    ...
    switch (issueType) {
      ...
      case "customer_unavailable":
        ...
        break;
           ...
           case "can not reach destination ":
        work.status = "cancelled ";
        work.remarks = remarks || "cannot reach ";
        await work.save();
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    emitWorkStatus(work); // socket event

    // =========================
    // TECHNICIAN NOTIFICATION
    // =========================

    const technicianId = work.assignedTechnician;

    if (technicianId) {

      const technician = await User.findById(technicianId)
        .select("firstName fcmTokens");
      ...
      if (technician?.fcmTokens?.length) {
        ...
        await sendPush(
          technician.fcmTokens,
          "Order Cancelled",
          `Client cancelled the ${work.serviceType} service request`,
          {
            role: "TECHNICIAN",
        ...
        console.log("✅ ORDER CANCEL PUSH SENT TO TECHNICIAN");

      } else {

        console.log("⚠️ Technician FCM token not found");
      ...
      await sendNotification(
        ...
        "technician",
        "Order Cancelled",
        `Client cancelled the booking for ${work.serviceType}`,
        "order_cancelled",
        `work-${work._id}`
      );
...
exports.rescheduleOrder = async (req, res) => {
  try {
    ...
    if (oldTechnicianId) {
      ...
      const oldTechUser = await User.findById(oldTechnicianId)
        .select("fcmTokens firstName");

      if (oldTechUser?.fcmTokens?.length) {

        await sendPush(
          oldTechUser.fcmTokens,
          "Job Rescheduled",
          `Client has rescheduled the ${work.serviceType} job`,
          {
            role: "TECHNICIAN",
        ...
        console.log("✅ RESCHEDULE PUSH SENT TO OLD TECHNICIAN");
      }
      ...
      await sendNotification(
        ...
        "technician",
        "Job Rescheduled",
        `Client has rescheduled the work: ${work.serviceType}`,
        "job_rescheduled",
        `work-${work._id}`
      );
    }
    ...
    const client = await User.findById(clientId).select("fcmTokens");
    ...
    if (client?.fcmTokens?.length) {
      ...
      await sendPush(
        client.fcmTokens,
        "Order Rescheduled",
        `Your ${work.serviceType} request has been rescheduled`,
        {
          role: "CLIENT",
      ...
      console.log("✅ RESCHEDULE PUSH SENT TO CLIENT");
    }
    ...
    for (const tech of matchingTechnicians) {
      ...
      const techUser = await User.findById(tech._id).select("fcmTokens");

      if (!techUser) continue;

      if (techUser?.fcmTokens?.length) {

        await sendPush(
          techUser.fcmTokens,
          "New Job Available",
          `New ${work.serviceType} job available near you`,
          {
            role: "TECHNICIAN",
            type: "job_rescheduled",
            workId: work._id.toString()
        ...
        console.log(`🚀 Job request sent to technician: ${tech._id}`);
      }
